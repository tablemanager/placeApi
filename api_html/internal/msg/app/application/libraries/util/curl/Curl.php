...
require 'StrUtil.php';
require 'Decoder.php';
require 'CaseInsensitiveArray.php';
...
class Curl
{
    ...
    {
        $this->jsonDecoder = '\Ticket\Decoder::decodeJson';
    ...
    {
        $this->xmlDecoder = '\Ticket\Decoder::decodeXml';
    }
    ...
    public function setDefaultDecoder($mixed = 'json')
    {
        if ($mixed === false) {
            ...
            if ($mixed === 'json') {
                $this->defaultDecoder = '\Ticket\Decoder::decodeJson';
            } elseif ($mixed === 'xml') {
                $this->defaultDecoder = '\Ticket\Decoder::decodeXml';
            }
        }
    }
    ...
    /**
     * @desc 배열을 오브젝트로 반환 v2 (arrayToObject => 공백이 있는경우 array로 retrun하기 때문에 아래와 같이 처리함.
     * @paramarray
     * @return object
     */
    function objectToArray2($object)
    {
        ...
        if (!empty($object)) {
            ...
            // 아래의 조건문을 empty 로 판단할 경우 0을 공백처리하는문제가 있음..
            foreach ($arrObj as $key => $val) {
                if (is_array($val) || is_object($val)) {
                    $out_arr[$key] = ($val == '') ? '' : $this->objectToArray2($val);
                } else {
                    $out_arr[$key] = ($val == '') ? '' : $val;
                }
            }
    ...
    public function arrayToObject($d)
    {
        if (is_array($d)) {
            ...
            return (object)array_map(array($this, 'objectToArray'), $d);
        } else {
            // Return object
            return $d;
        }
     ...
     * @param  $raw_headers
     *
     * @return \Ticket\CaseInsensitiveArray
     */
