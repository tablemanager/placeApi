 ...
 *
 * @param	$path_file{String} : 이미지의 절대경로 또는 상대경로
 * @name 	get_image_resource_from_file
 * @returns {Array}	성공시 리턴값 : 이미지 리소스 id 와 getimagesize 로 받아온 이미지 정보를 배열로 반환 실패시 리턴값 : 빈 배열 반환
 */
if ( ! function_exists('get_image_resource_from_file')) {
...
/**
 * 용도 :  image resouce 를 가지고 파일로 저장   gif 이미지는 $quality 의 영향을 받지 않음
 * @param	$im{String}	 ==> $im : 이미지 리소스 id
 * @param	$path_save_file{String}	 장될 파일의 절대 경로 또는 상대경로
 * @param	$quality{Int}	 저장되는 파일의 질을 결정 100 이하의 정수로 지정, 높을수록 질이 높음 생략하면 자동으로 기본값은 70
 * @param	$save_force{Int} 동일 경로에 이미 파일이 존재할때 0 이면 저장하지 않고 false 반환 ,
 * 1 이면 저장하지 않고 true 반환 2 이면 기존 것은 지우고 새로 저장 생략하면 자동으로 기본값은 0
 * @name 	save_image_from_resource
 * @returns {boolean}	성공시 리턴값 : 이미지 리소스 id 와 getimagesize 로 받아온 이미지 정보를 배열로 반환 실패시 리턴값 : 빈 배열 반환
 * @
 */
if ( ! function_exists('save_image_from_resource')) {
 ...
 * @param	$src_w{String} : 큰이미지의 너비, 0보다 큰 정수만 가능
 * @param	$src_h{String} : 큰이미지의 높이, 0보다 큰 정수만 가능
 * @param	$dst_size{String} : 작은 이미지의 정해진 너비 나 높이, 너비 일 경우 높이 반환, 높이 일 경우 너비 반환
 * @param	$rule{String} : $dst_size 의 값이 너비 인지 높이인지 지정
값으로는 width, height 가 올수 있다.
생략하거나 height 가 아니면 모두 width로 인식
 * @name 	get_size_by_rule
 * @returns {Array}	성공시 리턴값 : 0보다 큰 정수값 : 빈 배열 반환
 */
if ( ! function_exists('get_size_by_rule')) {
 ...
 * @param	$dst_w{String} : 작은이미지의 너비, 0보다 큰 정수만 가능
 * @param	$dst_h{String} : 작은이미지의 높이, 0보다 큰 정수만 가능
 * @param	$src_size{String} : 큰 이미지의 정해진 너비 나 높이, 너비 일 경우 높이 반환, 높이 일 경우 너비 반환 값으로는 width, height 가 올수 있다. 생략하거나 height 가 아니면 모두 width로 인식
 * @param	$rule{String} : $src_size 의 값이 너비 인지 높이인지 지정
 * @name 	get_bigsize_by_rule
 * @returns {Array}	성공시 리턴값 : 0보다 큰 정수값 : 빈 배열 반환
 */
if ( ! function_exists('get_bigsize_by_rule')) {
...
/**
 * 용도 : 본의 리소스를 가지고 주어진 조건으로 리사이즈 처리한 이미지 리소스를 생성
 * $dst_w 와 $dst_h 모두 값이 0이 될수 없음
 * 둘다 0보다 클 경우, 강제 리사이즈하여 썸네일 리소스 생성
 * 둘중 하나가 0 이면, 0이 아닌 쪽을 기준으로 정비율로 리사이즈 하여 썸네일 생성
 *
 * @param	$src{String}  : 원본의 리소스 id
 * @param	$src_w{String}  : 원본의 너비
 * @param	$src_h{String}  : 원본의 높이
 * @param	$dst_w{String}  : 생성할 썸네일의 너비, 0 이상의 정수
 * @param	$dst_h{String}  : 생략 가능하며 생략시에는 자동으로 0으로 값이 들어감
 * @name 	get_image_resize
 * @returns {Array}	성공시 리턴값 ? 썸네일 리소스 id : false
 */
 ...
 *
 * @param	$src{String}  : 원본의 리소스 id
 * @param	$src_w{Int}  : 원본의 너비
 * @param	$src_h{Int}  : 원본의 높이 생성할 썸네일의 너비, 0 이상의 정수
 * @param	$dst_h{Int}  : 생성할 썸네일의 높이, 0 이상의 정수 생략 가능하며 생략시에는 자동으로 0으로 값이 들어감
 * @param	$pos_width{Int}  : 너비를 기준으로 크롭할때 어느부분을 크롭할지 지정
==> 1 일경우에는 왼쪽을 기준으로 크롭
==> 2 일경우에는 중앙을 기준으로 크롭
==> 3 일경우에는 오른쪽을 기준으로 크롭
==> 생략가능하며 생략시에는 자동으로 2 로 값이 들어감
 * @param	$pos_width{Int} : 높이를 기준으로 크롭할때 어느부분을 크롭할지 지정
==> 1 일경우에는 상단을 기준으로 크롭
==> 2 일경우에는 가운데를 기준으로 크롭
==> 3 일경우에는 하단을 기준으로 크롭
==> 생략가능하며 생략시에는 자동으로 2 로 값이 들어감
 * @name 	get_image_cropresize
 * @returns {String}	성공시 리턴값 ? 썸네일 리소스 id : false
 */
if ( ! function_exists('get_image_cropresize')) {
 ...
 * @param	$src{String}==> : 원본의 리소스 id
 * @param	$src_w{Int}==> : 원본의 너비
 * @param	$src_h{Int}==> : 원본의 높이
 * @param	$path_mark_file{String}==> : 워터마크로 사용될 이미지파일의 전체경로 또는 상대경로
 * @param	$pos{Int}==> : 워터마크를 찍을 포지션을 숫자로 지정
==> 1 일경우에는 상단 왼쪽에 한번만
==> 2 일경우에는 상단 오른쪽에 한번만
==> 3 일경우에는 하단 왼쪽에 한번만
==> 4 일경우에는 하단 오른쪽에 한번만
...
==> 10 일 경우에는 전체를 반복해서
==> 그 외의 값은 4로 처리
 * @param	$sharpness{Int}==> : 워터마크의 선명도, 0부터 100 까지의 정수만 가능
==> 100 일 경우에는 투명이미지를 사용하는것으로 간주, 투명이미지로 워터마크 처리
 * @param	$padding{Int}==> :  워터마크 사이의 간격, 생략가능하며 생략시 자동으로 0 로 값이 들어감
 * @name 	proc_watermark
 * @returns {Array}	성공시 리턴값 ? true: false
 */
if ( ! function_exists('proc_watermark')) {
 ...
 * 둘중 하나가 0 이면, 0이 아닌 쪽을 기준으로 정비율로 리사이즈 하여 썸네일 생성
 *
 * @param 	$url{String}: 외부 이미지 주소
==> $referer : 리퍼러, 생략 가능, 생략시 기본 호스트가 들어감
 * @name 	get_remote_image
 * @returns {Array}	성공시 리턴값 ? 이미지 파일 내용: false
 */
 ...
 * @param	$path_src_file_or_url{String} : 원본파일의 절대경로 또는 상대경로 또는 주소
 * @param	$path_save_file{String} : 썸네일을 저장할 절대경로 또는 상대경로
 * @param	$save_w{Int} : 만들 썸네일의 너비
 * @param	$save_h{Int} : 만들 썸네일의 높이, 생략 가능하며 생략시 기본값은 0
 * @param	$options{Array} :  함수 내부에 정의된 변수들의 값을 변경할때 사용, 배열형태, 생략가능하며 생략시 기본값은 빈배열(Array())
...
//     $save_force = 1;//저장형태 : 파일 덮어씌움
        ...
        if (preg_match("`^http`i", $path_src_file_or_url)) {
            ...
            //가자고 , 갤러기 전용 높이 반 퀄리티 5프로 떨어트림
            if (empty($save_w) && empty($save_h)) {
                $save_w = (int)$src_w / 2;
                $save_h = (int)$src_h / 2;
                $save_quality = 70;
                $save_force = 1;
