...
$aqua_barcode = (string)$Xres->aqua_barcode;
$shinhwa_qrcode = (string)$Xres->shinhwa_ticketno;

if($resCd == "0000"){
  header("HTTP/1.0 200");
  $res = array("Result"=>"0000","Msg"=>"성공","Barcode" =>$resBarcode, "aqua_barcode"=>$aqua_barcode, "shinhwa_qrcode"=>$shinhwa_qrcode);
  echo json_encode($res);
  exit;
}else{
  header("HTTP/1.0 400 Bad Request");
  $res = array("Result"=>"4000","Msg"=>"필수 파라미터 오류 ep-rtn");
  echo json_encode($res);
  exit;
}
