...
// 비어있으면, LMS로 보냄..
if ($Send_type == "MMS" && empty($item['mmsFile'])) {
//    $Send_type = 'LMS';
}
// ====================================
// 어떠한 타입으로 들어왔는지..
...
switch($Send_type) {
    ...
    // 카카오알림톡 전송 실패시 LMS로 전송
    // $item에 LMS 내용이 별도로 추가되어 있음
    case 'KAKAOV2':
        $res = Send_KakaoV2($item, $Send_type);
        //print_r($res);       
        if($res['result'] == TRUE){
            $Jres = json_decode($res['data'], TRUE);
            if($Jres[0]['code'] == "M000" ||$Jres[0]['code'] == "M001"){
                // LMS(SMS)가 전송되었으므로 내용을 문자 내용으로 변경함
                $item['msgText'] = $item['smsText'];
            }
        }
