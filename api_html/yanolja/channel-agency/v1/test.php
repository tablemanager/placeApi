...
// pagesize 최대 2000건, pageNumber는 0이상
...
if($aclmode == "IP"){
  ...
  if(!in_array(get_ip(),json_decode($authrow->accessip,false))){
      ...
      $res = array("Result"=>"4100","Msg"=>"IP 인증 오류");
...
switch($itemreqs[3]){
    ...
    default:
        ...
        $res = array("Result"=>"400000","Msg"=>"파라미터 오류 : ".$itemreqs[3]);
        echo json_encode($res);
}
...
function get_cancel($cancel,$pageNumber,$pageSize){
    ...
    // ======== Response Body
          ...
          $cntOrders = count($orders); // 날짜 조회시 총 주문갯수
          $cntPages = count($pageSize); // 몇 개 검색할지
          $totCnt = ($cntOrders - ($cntOrders % $cntPages)) / $cntPages; //총 페이지 수


          $pages = array(
            'pageNumber' => $pageNumber,
            'pageSize' => $pageSize, //limit 몇개검색할지?
            'totalElementCount' => $cntOrders,
            'totalPageCount' => $totCnt
          );
...
function get_reverted($reverted,$pageNumber,$pageSize){
    ...
    while($row = $res->fetch_object()){
        $revertdate = explode(" ",$row->syncmsg);
        // var_dump($canceldate);
        // exit;
