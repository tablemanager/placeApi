...
if(strlen($para) < 12){
    ...
    switch($res['state']){
        case 'N':
        case 'Y':
            use_sticket($res['couponno']);
            useev($res['couponno']);
            usecouponno($res['couponno']);
            $track = $xml->addChild('code',1);
            $track = $xml->addChild('message',"사용처리성공");
        break;
        /*
        case 'Y':
            $track = $xml->addChild('code',0);
            $track = $xml->addChild('message',"사용처리실패 / 기사용코드");
        break;
        */
        case 'C':
            $track = $xml->addChild('code',0);
            $track = $xml->addChild('message',"사용처리실패 / 취소코드");
        break;
        default:
            $track = $xml->addChild('code',0);
...
// Sticket 사용처리
function use_sticket($no){

    global $conn_cms3;

    $uqry = "update spadb.pcms_extcoupon set state_use= 'Y', date_use = now() where state_use= 'N' and couponno = '$no' limit 1";
...
function useev($no){
	...
	$curl = curl_init();
    $url = "http://115.68.42.8/everland/process?PIN_NO=".$no;
