...
function get_order($orderno){
    ...
    while($row = $res->fetch_object()){
        ...
        $splited_orderno = explode("-",$row->ch_orderno);
        $chatbot_orderno = $row->ch_orderno;
        $order_seq = $row->id;
...
function get_couponinfo($_faccd,$_mode,$_val){
    ...
    if($_faccd != '4249' && $_faccd != '2781' && $_faccd != '9' && $_faccd != '76' && $_faccd != '4308'){
        $result = [
            "Code"=>9999,
            "Msg"=>"현재 제공되지 않는 시설입니다."
        ];
        header("HTTP/1.0 200");
        echo json_encode($result);
    }else{
          ...
          for($i=0;$i<count($optnms);$i++){
              ...
              if($iteminfo->item_nm == null) {
                ...
                $optitems[]= array(
                    ...
                    //"itemSchedule" => array($expDate) // 상품관리의 판매 종료일
