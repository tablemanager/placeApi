...
// 호출 URL : https://gateway.sparo.cc/assets/dev/qr_dev.php?val=EL5000650110616920.jpg
...
$qr_dev_dir = "/home/sparo.cc/api_html/assets/temp/";

//$code = 'EL0000650110616920';
...
// ===============================================
// 기본 QR 그려내기.  /EL5000650110616920 - 일반건 / EL5000650184759020 - 추가이미지
// ===============================================
function img_make($code, $debug = false)
{
    ...
    // 크기, 마진값 순으로
    QRcode::png($code, $debug == false ? $qr_dev_dir . $code . '.png' : false, QR_ECLEVEL_H, 9, 5);
}
...
function combine_img($code, $img_url = '')
{
    ...
    $qr = imagecreatefrompng($qr_dev_dir . $code . '.png');
    $qr_code = imagecreatefrompng($qr_dev_dir . $code . "_qr.png");
    $text = imagecreatefrompng($qr_dev_dir . $code . "_bottom.png");

    // 이미지의 따른 사이즈 변화를 위하여 설정함.
    list($top_width, $top_height) = getimagesize($qr_dev_dir . $code . '.png');
    list($middle_width, $middle_height) = getimagesize($qr_dev_dir . $code . "_qr.png");
    list($bottom_width, $bottom_height) = getimagesize($qr_dev_dir . $code . "_bottom.png");
...
//    unlink($qr_dev_dir . $code . '.png');
//    unlink($qr_dev_dir . $code . "_qr.png");
//    unlink($qr_dev_dir . $code . "_bottom.png");
...
function get_text($code, $text, $debug = false)
{
    ...
    $qr_path = $debug == true ? false : $qr_dev_dir . $code . "_qr.png";
    $text_path = $debug == true ? false : $qr_dev_dir . $code . "_bottom.png";
    if ($debug) Header("content-type: image/png");
...
//    $pcms_sticket_sql = @"SELECT pcms_id, bar_option, addimg
//                            FROM spadb.`pcms_sticket`
//                            WHERE `pcms_id` = '00000' and code_visible = 'Y' and mms_visible = 'Y'
//                        ";
...
function get_img($code,$url)
{
    ...
    $path_filename = $qr_dev_dir . $filename;
