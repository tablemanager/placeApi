...
// 호출 URL : https://gateway.sparo.cc/assets/dev/qr_dev.php?val=CB50009824WU2JBA70.jpg
...
function get_text($code, $text, $debug = false)
{
    ...
    if (is_array($text)) {
        ...
        //imagettftext($img, 18, 0, $x, 32, $tc, $font, $text['bartext']);
				imagettftext($img, 11, 0, $x, $y, $tc, $font, $text['bartext']);
...
function get_bar_option($code)
{
    ...
    $pcms_extcoupon_sql = @"SELECT couponno,sellcode
                                ...
                                WHERE `couponno` = 'CB50009824WU2JBA70'
                           ";
    $result = $conn_cms3->query($pcms_extcoupon_sql);
    $sellcode = $result->fetch_array()['sellcode']; // PCB50009824_2392
