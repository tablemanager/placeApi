...
/*
                $sqlcpn = "SELECT order_id, state 
                           FROM spadb.ordermts_coupons
                           WHERE couponno  like '16%' limit 5  -- 시험용쿼리
                           -- WHERE couponno = '$v_val'        -- 실 사용 쿼리
                           ";
                $rescpn = $conn_cms3->query($sqlcpn);
                $arCpn = array();
                $orderids = "";
                while($rowcpn = $rescpn->fetch_object())
                {
                    $arCpn[$rowcpn->order_id] = $rowcpn;

                    if (strlen($orderids)==0){
                        $orderids = $rowcpn->order_id;
                    } else {
                        $orderids = $orderids."', '".$rowcpn->order_id;
                    }
                }
*/
                // 주문정보 조회
                ...
                $sql = "SELECT *
	                ...
	                WHERE true
	                    AND grmt_id = '$v_ch' 
                            ANd usegu_at like '".date('Y-m-d')."%'   -- 오늘 사용처리된 쿠폰
