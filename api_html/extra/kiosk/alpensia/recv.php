...
$logsql = "insert cmsdb.extapi_log set apinm='alpensia/recv', chnm='ALPENSIA request[$type]', tran_id='$tranid', ip='".get_ip()."', logdate=now(), apimethod='$apimethod', header='".addslashes(json_encode($apiheader))."', querystr='$para'";
...
*/
      ...
      // 오션700 시즌권 고객정보 접수일 경우 핀번호 사용처리를 하지 않는다.
      $isOcean700pass = $_GET['internal'];
      if (isset($isOcean700pass) && $isOcean700pass == "ocean700pass"){
          // cms.alpensia_extcoupon 테이블에 사용처리 플레그 설정하지 않는다.
          // 주문건은 사용처리 하지만, 핀번호는 입장할때 사용처리 해야 함
          $rtn = true;
      }else{
