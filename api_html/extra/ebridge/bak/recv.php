...
/*
*
* 이브릿지 사용처리 리시버:이브릿지는 사용처리 통지가 없으므로 이 인터페이스는 사용하지 않음.
*
*/
exit;
...
$logsql = "insert cmsdb.extapi_log set apinm='ebridge/recv', chnm='이브릿지 request[$type]', tran_id='$tranid', ip='".get_ip()."', logdate=now(), apimethod='$apimethod', header='".addslashes(json_encode($apiheader))."', querystr='$para'";
...
$_row = $conn_rds->query("select id, orderno, state from cmsdb.lpensia_extcoupon where barcode= '$barcode' limit 1")->fetch_object();
...
*/
      $usql = "update cmsdb.lpensia_extcoupon set state = 'Y', useDt =  now() where state='N' and barcode= '$barcode' limit 1";
  ...
  case 'unuse':
    ...
    $unused = false;
    if(strlen($_row->orderno) > 11){
        $usql = "update cmsdb.lpensia_extcoupon set state = 'N', useDt = null where state='Y' and barcode= '$barcode' limit 1";
        $ures = $conn_rds->query($usql);
        // 쿼리가 정상적으로 실행된 경우에 회수 처리 정상인 것으로 판단
        if($ures) $unused = true;

        // 주문번호에 연결된 모든 쿠폰번호가 미사용일 경우에만 주문테이블 정보를 미사용으로 변경
        $osql = "select * from cmsdb.lpensia_extcoupon where orderno='$_row->orderno' and state='Y'";
...
curl_setopt_array($curl, array(
  CURLOPT_URL => 'https://gateway.sparo.cc/extra/kiosk/lpensia/cancel/'.$couponno,
