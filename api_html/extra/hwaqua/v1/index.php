...
/**
 *
 * @brief 스마트인피니(한화63아쿠아) 제공용 티켓 사용처리/복원(회수) 인터페이스
 *
 * @author ila
 * @date 22.06.09
 *
...
$pin_hw = $decodedreq->coupon_no;
list($microtime,$timestamp) = explode(' ',microtime());

$tranid = $timestamp . substr($microtime, 2, 3);
//nsertLog($apiheader,$apimethod,$para,$jsonreq);
...
$orderno = $hw63aquaDB->getOrderno($pin_hw);

$order = $hw63aquaDB->getStateByOrderno($orderno);

switch($order_div){
    case '51': // spadb.ordermts + 해당 시설 테이블 사용처리
        $state = $hw63aquaDB->getState($pin_hw);
        // 주문정보에서 미사용일 경우에만 사용처리하며, 한화아쿠아 전용 테이블에 사용처리
        if($state == 'N' && $order->state == '예약완료' && $order->usegu == '2'){
            // 쿠폰번호 한화아쿠아 전용 테이블 사용처리
            $updateResult = $hw63aquaDB->use($ticket_code);
            if($updateResult){
                // 쿠폰번호 사용처리
                usecouponno($ticket_code);
                // spadb.ordermts 사용처리 플레그 갱신
                $hw63aquaDB->updateUsegu1($orderno);
                $state = 'S';
            } else {
                $state = 'E';
            }
        }
        // 주문정보에서 사용일 경우에는 주문정보에서만 사용처리 된 것이므로, 한화아쿠아  전용 테이블에서 사용처리
        elseif($state == 'N' && $order->state == '예약완료' && $order->usegu == '1'){
            // 한화아쿠아 서울랜드 전용 테이블 사용처리
            $updateResult = $hw63aquaDB->use($ticket_code);
            if($updateResult){
                // 쿠폰번호 사용처리
                usecouponno($ticket_code);
    ...
    case '50': //복원 처리
        ...
        if($state == 'Y' && $order->state == '예약완료' && $order->usegu == '1'){ // 사용일 경우에만 회수처리
            $updateResult = $hw63aquaDB->restore($ticket_code);


            if($updateResult){
                $usedPins = $hw63aquaDB->getUsedPins($orderno);
                if(empty($usedPins)){
                    $hw63aquaDB->updateUsegu2($orderno);
                }
                $state = 'S';
            } else {
                $state = 'E';
