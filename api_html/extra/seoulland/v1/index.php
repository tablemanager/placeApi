 ...
 * @author Jason
 * @date 22.03.24
 * 20231120 tony https://placem.atlassian.net/browse/P2CCA-444 [서울랜드] 쿠폰별 사용건 조회 확인 요청건
 * 사용처리 함수를 구버전에서 신버전으로 변환 : 적용 시각 Mon Nov 20 15:04:54 KST 2023
 */
...
switch($order_div){
    case '51': // spadb.ordermts + 해당 시설 테이블 사용처리
        ...
        if($state == 'N' && $order->state == '예약완료' && $order->usegu == '2'){
            ...
            if($updateResult){
                ...
                // 20231120 tony https://placem.atlassian.net/browse/P2CCA-444 [서울랜드] 쿠폰별 사용건 조회 확인 요청건
                // 응답처리를 하고 싶은데 생략되어 있음
                usecouponnov2($pin_seoul);
    ...
    case '50': //복원 처리
        ...
        if($state == 'Y' && $order->state == '예약완료' && $order->usegu == '1'){ // 사용일 경우에만 회수처리
            ...
            if($updateResult){
                // 서울랜드DB에서 쿠폰별 미사용 상태로 갱신
                $usedPins = $seoullandDB->getUsedPins($orderno);

                // 20231121 tony
                // cm에서 쿠폰별 미사용 상태로 갱신
                // 사용시간은 유지해서 복원된 건지 확인할 수 있도록 한다.
                $ucsql = "update spadb.ordermts_coupons set state='N' where couponno = '$pin_seoul' and order_id = '{$order->id}' limit 1";
                // json 응답이 나가야 하므로 출력 하면 안됨
                //echo $ucsql;echo "\n";
                $ucres = $conn_cms3->query($ucsql);
                // json 응답이 나가야 하므로 출력 하면 안됨
                //echo "CM 쿠폰 복원 결과 [$ucres] [{$conn_cms3->affected_rows}]\n";
                
                if(empty($usedPins)){
                    // cm 쿠폰별 상태가 모두 미사용인 경우 주문건을 미사용으로 전환한다.
                    $scsql = "select * from spadb.ordermts_coupons where order_id = '{$order->id}' and state = 'N'";
                    $scres = $conn_cms3->query($scsql);
                    $cntusegu2 = $conn_cms3->affected_rows;

                    // 미사용 갯수가 주문 수량과 동일한 경우
                    if ($order->man1 == $cntusegu2){ 
                        // CM에 주문건을 미사용 상태로 복원
