...
// 20230310 tony https://placem.atlassian.net/browse/PM2201COBBP-54 [3월][3순위]파라다이스시티] 바코드리딩(사용처리) 주중/주말 구분 확인요청건
// 없는 쿠폰번호가 파라미터로 호출될때 json_result['status'] 에 값이 없어서 정상인것 처럼 처리되고 paradian_reservation 테이블에서 못찾으므로 응답에 ''로 회신함.
// 없는 번호라고 코드를 넣고 싶지만 현재 사용중인 시설에서도 협의/수정이 있어야 되므로 수정하지 않음
...
2200    주말(금토일), (대체)공휴일에 평일권 사용불가
2300    주말권(금토일, (대체)공휴일)은 평일에 사용불가
2400    공휴일정보 없음
2410    티켓유형 설정 없음1
2420    티켓유형 설정 없음2
 */
...
// 로그파일
$fnm = "/home/sparo.cc/api_html/extra/para/txt/".date('Ymd')."para2.log";
...
//print_r($row);
        ...
        // 티켓 유형 체크
        // 주말권/평일권/공통권 체크
        $chktk = chkTkTp($orderInfo->itemmt_id);

        // 100 : 티켓유형과 주중/주말이 일치함
        // 201 : 옵션 설정 없음
        // 202 : 옵션의 환경설정 없음
        // 301 : 주중권은 주말에 사용할수 없음
        // 302 : 주말권은 평일에 사용할수 없음 
        switch($chktk){
        case "201":
            // 티켓유형 정보 없음
            $json_result['branch_cd'] = '1000';
            $json_result['status'] = '2410';  // 티켓유형 설정 없음1
            $json_result['method'] = $itemreq[1];
            $json_result['barcode'] = $itemreq[0];
            $json_result['tran_id'] = $tranid;
            $_log_header = "use4";
        ...
        case "202":
            // 주중권은 주말에 사용할수 없음
            $json_result['branch_cd'] = '1000';
            $json_result['status'] = '2420';  // 티켓유형 설정 없음2
            $json_result['method'] = $itemreq[1];
            $json_result['barcode'] = $itemreq[0];
            $json_result['tran_id'] = $tranid;
            $_log_header = "use4";
        ...
        case "301":
            // 주중권은 주말에 사용할수 없음
            $json_result['branch_cd'] = '1000';
            $json_result['status'] = '2200';  // 주말(금토일), (대체)공휴일에 평일권 사용불가
            $json_result['method'] = $itemreq[1];
            $json_result['barcode'] = $itemreq[0];
            $json_result['tran_id'] = $tranid;
            $_log_header = "use4";
        ...
        case "302":
            // 주말권은 평말에 사용할수 없음
            $json_result['branch_cd'] = '1000';
            $json_result['status'] = '2300';  // 주말권(금토일, (대체)공휴일)은 평일에 사용불가
            $json_result['method'] = $itemreq[1];
            $json_result['barcode'] = $itemreq[0];
            $json_result['tran_id'] = $tranid;
            $_log_header = "use4"; 
        ...
        default:
            // 알수없는 에러
             $json_result['branch_cd'] = '1000';
             $json_result['status'] = '2000';  // 알수없는 에러
             $json_result['method'] = $itemreq[1];
             $json_result['barcode'] = $itemreq[0];
             $json_result['tran_id'] = $tranid;
             $_log_header = "use4";
...
function dellog(){
    ...
    //echo $ldate;
    $dellog = "/home/sparo.cc/api_html/extra/para/txt/".$ldate."para2.log";
...
function isHolyday(){
    ...
    // 양력 : 신정(01-01), 삼일절(03-01), 어린이날(05-05), 현충일(06-06), 광복절(08-15), 개천절(10-03), 한글날(10-09), 성탄절(12-25)
    // 음력 : 설날, 추석을 양력으로 변환하여 등록해야 함.
    // 선거일, 임시공휴일, 대체공휴일을 등록해야 함.
    $arHolyday = array(
        // 2023년
        '2023-01-01','2023-03-01','2023-05-05','2023-05-27','2023-06-06','2023-08-15',
        ...
        '2025-05-06','2025-06-06','2025-08-15','2025-10-03',
        '2025-10-05','2025-10-06','2025-10-07','2025-10-08','2025-10-09','2025-12-25',
    );

    if(in_array($today, $arHolyday)){
        return true;
    }else{
        return false;
    }
...
// 301 : 주중권은 주말에 사용할수 없음
// 302 : 주말권은 평일에 사용할수 없음
function chkTkTp($itemmt_id){
    global $conn_cms;

    // 시험기간 이전이면 기존루틴을 그대로 사용한다.
    // 20230501 오픈 예정이므로 시험이 끝나면 날짜를 5월로 변경한다.
    //  원더박스_국내(4028) 는 5월부터 주중/주말권 셋팅이됨(이전에는 세팅 안함)
    // 파라다이스시티_국내(4026) 도 시험일 현재 시험상품만 주중/주말권 설정되어 있음
    // 4월 10일 오전 9시부터 10시 50분 사이에만 시험이 가능한다.
    // 62052 주중권
    // 62053 주말권
    // 62054 공통권
    if(
        date("Y-m-d H:i:s") < "2023-04-11 09:00:00" ||
        date("Y-m-d H:i:s") > "2023-04-11 10:55:00"
        ){
        // 주중/주말/공휴일 상관없이 모두 사용 가능
        return "100";
    } 

    // 20230316 tony https://placem.atlassian.net/browse/PM2201COBBP-54  바코드리딩(사용처리) 주중/주말 구분 확인요청건
    // 보통권/주중권/주말권 확인 루틴 시작
    $tktypesql = "SELECT * FROM pcmsdb.cms_admin_assets WHERE TRUE
                        AND tcode = 'PC'
                        AND ccode in (
                            SELECT ccode FROM pcmsdb.cms_coupon 
                            WHERE TRUE
                            AND ctype = 'PC'
                            AND use_sdate <= DATE_FORMAT(now(), '%Y-%m-%d %H:%i:%s')
                            AND use_edate >= DATE_FORMAT(now(), '%Y-%m-%d %H:%i:%s')
                            AND items_id = '$itemmt_id'
                        )";
    $restktype = $conn_cms->query($tktypesql);
    $rowtktype = $restktype->fetch_object();

    if(!empty($rowtktype)){
        // {"pm_tcode":"PC","pm_ccode":"PP61444_1064","itemtype":"01","sex":"M","itemcode":"4460822683","price":"18400","tickettype":"NORMAL"}
        // tickettype : 없거나 NORMAL (보통권), WEEKDAY(주중권), HOLYDAY(주말(금토일), 공휴일)
        if(isset($rowtktype->cconfig)){
            $cconfig = json_decode($rowtktype->cconfig);
            $tickettype = $cconfig->tickettype;

            // 보통권, 기본값
            if($tickettype == "NORMAL"){
                // 주중/주말/공휴일 상관없이 모두 사용 가능
                return "100";
            }else if($tickettype == "WEEKDAY"){
                // 주중권 : 주말도 휴일도 아니어야 한다.
            ...
            }else if($tickettype == "HOLYDAY"){
                // 주말(금토일), (대체)공휴일에만 사용가능
                if (isWeekend() || isHolyday()){
                    // 주말이거나(금토일), (대체)공휴일 이므로 에러발생 안시킴
                    return "100";
                }else{
                    return "302";
                }
    ...
    // 티켓유형을 알수 없음. 쿠폰생성요청 메뉴에서 * 눌러 옵션 설정 "사용일구분" 값 설정
    }else{
        return "201";
    }
}
