        ...
        if(!$itemurow->id){
           ...
           // 조회 결과가 없을시
            ...
            if($sellcode){
                ...
                // 이브릿지
                if($typefac == "EBG"){
                    $ebgreq =  array("orderno"=>$info->orderNo,
                        "qty" => 1,
                        "usernm"=>trim($info->userName),
                        "userhp"=>trim($info->userHp),
                    );

                    $cpno = get_ebridgeno($itemcode, $ebgreq);
                } 
            }
...
// 20240122 tony https://placem.atlassian.net/browse/P2CCA-466 5. [해외채널] 이브릿지 (4359) 주문등록 시 쿠폰번호 전송 개발
// 이브릿지 쿠폰 생성
function get_ebridgeno($itemcode,$_req){

    $curl = curl_init();

    curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://gateway.sparo.cc/extra/ebridge/order/'.$itemcode,
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
    ...
    [0] => stdClass Object
        (
            [name] => K공항리무진 승차권 (1인)
            [coupon_num] => 8392800001662377
            [valid_start_date] => 1970-01-01
            [valid_end_date] => 2023-12-28
            [short_url] => http://qt7.kr/a9n3c
        )
    
    )
    */

    return (isset($result[0]) && isset($result[0]->coupon_num))?$result[0]->coupon_num:false;
}
