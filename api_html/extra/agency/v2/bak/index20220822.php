        ...
        if(!$itemurow->id){
           ...
           // 조회 결과가 없을시
			...
			if(strlen($itemcode) > 5){
		       //  send_notice("판매채널 연동","판매채널에서 잘못된 코드 설정 - {$cpname} / {$cpcode} - $itemcode");
            }else{
		       //  send_notice("판매채널 연동","플레이스엠에서 판매채널 연동누락 - {$cpname} / {$cpcode} - $itemcode");
			}
            ...
            if($sellcode){
                ...
                if($typefac == "HW"){
                    $hwworld = array();
                    $hwworld[] = array("ORDERNO"=>$info->orderNo,
                        "SELLCODE"=>$itemcode,
                        "UNIT"=>$info->Qty,
				...
				// 대명, 롯데B2B - 22.08.08 롯데B2B 구분코드 추가 ila 
				if($typefac == "DM" or $typefac == "LWB2B") {
					$cpno = "0";
				}
