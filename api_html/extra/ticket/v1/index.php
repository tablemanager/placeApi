...
// 20240716 tony https://placem.atlassian.net/browse/XI2404-1
// [어뮤즈스파]진주점 키오스크 연동 개발 
// 업체:중흥에스클래스(3948)
// 시설:어뮤즈스파 진주점(4371)
...
$grmtId = $authrow->cp_grmtid;  // 업체코드
$jpmtId = $authrow->cp_code;    // 시설코드
		...
		switch($grmtId){ // 업체 아이디 분기
            ...
            case '3948': // 강릉또강릉
                ...
                if(!empty($scrow)){
                    ...
                    $sosql = "select *, AES_DECRYPT(UNHEX(hp),'Wow1daY') dhp from spadb.ordermts where id='{$scrow->order_id}' and grmt_id='$grmtId' and usedate>='".date("Y-m-d")."'";
                    ...
                    if(!empty($sorow)){
                        ...
                        //$info[] = array("orderno" => $sorow->orderno, "couponno" => $couponno, "expdate" => $sorow->usedate, "state" => $sorow->state, "ustate" => $ustate, "cusnm" => $sorow->usernm, "cushp" => $sorow->dhp, "cusopt" => "");
                        $info[] = array("orderno" => $sorow->orderno, "couponno" => $couponno, "expdate" => $sorow->usedate, "state" => $sorow->state, "ustate" => $ustate, "cusnm" => $sorow->usernm, "cusopt" => "");
                    }else{
                        $state = '';
                    }
                }else{
