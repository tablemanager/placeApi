<?php
// -rw-rw-r-- 1 placedev placedev 11979 Jul 18  2022 req.php
...
$ORDER_NO = trim($_GET['order_no']);		//주문번호
$AR_COUPON_NO = explode(",",trim($_GET['coupon_no']));		//쿠폰번호 (','로 구분)
...
$STATUS_DIV = trim($_GET['status_div']);	//상태값 (','로 구분)
$RESULT_DATE = trim($_GET['result_date']);	//사용일시 (','로 구분)
...
$qrysql = "insert pcmsdb.log_asanorders set mode = '$STATUS_DIV', qrystr = '$qrystr', regdate = now(), ip ='$sip(3)'";
				...
				case 'I':	//사용
					...
					switch($synctype){
						...
						case 'COUPANG':
							...
							$NSPRuseqry2="update spadb.ordermts_coupons set
									state = 'Y', dt_use ='$RESULT_DATE'
									where order_id = '{$Orow->PCMS_orderid}' and state != 'Y' limit 1";
							$NSPRuseres2 = $conn_cms3->query($NSPRuseqry2);
					...
					if($uc_rows == 0){
						...
						switch($synctype){
							...
							case 'COUPANG':
								...
								$NSPRuseqry2="update spadb.ordermts_coupons set
										state = 'N', dt_use = null
										where order_id = '{$Orow->PCMS_orderid}' and state != 'N' limit 1";
								$NSPRuseres2 = $conn_cms3->query($NSPRuseqry2);
